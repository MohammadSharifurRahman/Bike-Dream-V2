...
@api_router.post("/seed-sample-ratings")
async def seed_sample_ratings():
    """Add sample ratings and reviews to motorcycles for demonstration"""
    try:
        ...
        # Add ratings to random motorcycles
        import random
        updated_count = 0
        
        for motorcycle in motorcycles:
            # Add ratings to roughly 60% of motorcycles
        ...
        return {
            "message": f"Successfully added sample ratings to {updated_count} motorcycles",
            "total_motorcycles": len(motorcycles),
            "ratings_added": updated_count
        }
        
    except Exception as e:
        logger.error(f"Error seeding sample ratings: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error seeding sample ratings: {str(e)}")
